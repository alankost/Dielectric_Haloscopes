...
CO2_n_complex = functools.partial(CO2_n_v_TP.ncomplexHT, T=100, P=5)
n_gas = CO2_n_complex
eV_to_omega = 1.519e15
omega0_CO = 0.266 * eV_to_omega
omega0_CO2 = 0.2915 * eV_to_omega
omega0 = omega0_CO2     # resonant frequency of stack

start_time = time.perf_counter()

# # plot |BL|^2 vs. omega (wide shot)
#
# omega_wide = np.linspace(0.95, 1.05, 100001) * omega0
# boost_wide = stacks.hw_boost(omega_wide, omega0, n_glass, n_gas, num_layers, n2_real0=n_gas0)
...
n_line_alpha = 0.5
n_linewidth = 1.25
omega_for_n = omega0 * (1 + np.linspace(-1, 1, 10000) / 100)
n_of_omega = n_gas(omega_for_n)
...
n_of_omega0 = n_gas(np.array(omega0))[0]
...
omega_closeup_norm = np.linspace(0.99, 1.01, 5001)
omega_closeup = omega_closeup_norm * omega0
complex_n = n_gas(omega_closeup)
